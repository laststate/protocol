...
func SealWithNonce(plain []byte, ring Keyring, encrypt bool, nonce []byte) ([]byte, error) {
	...
	if env.Flags&^uint8(FlagTruncated|FlagCompressed) != 0 {
	...
	preserve := env.Flags & (FlagTruncated | FlagCompressed)
...
func Open(raw []byte, ring Keyring) ([]byte, error) {
	...
	if env.Flags&FlagAEAD != 0 {
		return openAEAD(raw, env, ring)
	}
	return openHMAC(raw, env, ring)
}

func openAEAD(raw []byte, env Envelope, ring Keyring) ([]byte, error) {
...
func openHMAC(raw []byte, env Envelope, ring Keyring) ([]byte, error) {
...
func MetadataKeyID(raw []byte) (uint32, bool) {
	env, err := Validate(raw)
	if err != nil || env.Flags&FlagAEAD == 0 {
		return 0, false
	}
